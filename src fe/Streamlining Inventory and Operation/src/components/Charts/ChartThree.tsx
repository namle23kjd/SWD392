...
const ChartThree: React.FC = () => {
  ...
  const options: ApexOptions = {
    ...
    colors: ['#6577F3', '#8FD0EF', '#0FADCF', '#FF5733', '#33FF57', '#FF33A1', '#8B00FF', '#FF4500', '#FFD700', '#00CED1']
    , // Always defined
    labels: state.labels,
    legend: {
      show: false,
  ...
  async function handleFetchData() {
    const response = await getOrdersByPlatform();

    // Update series and labels dynamically based on chartData
    const seriesData = response.map((data: any) => data.orders); // Assuming 'orders' holds the relevant data
    const labelsData = response.map((data: any) => data.platform); // Assuming 'platform' holds platform names
